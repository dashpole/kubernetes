...
func TestScaleDownOldReplicaSets(t *testing.T) {
	...
	for i := range tests {
		...
		c, err := NewDeploymentController(ctx, informers.Apps().V1().Deployments(), informers.Apps().V1().ReplicaSets(), informers.Core().V1().Pods(), kc, tracing.NewNoopTracerProvider().Tracer(""))
		if err != nil {
