...
func updateReplicaSetStatus(c appsclient.ReplicaSetInterface, rs *apps.ReplicaSet, newStatus apps.ReplicaSetStatus, endTrace bool) (*apps.ReplicaSet, error) {
	...
	for i, rs := 0, rs; ; i++ {
		...
		rs.Status = newStatus
		if endTrace {
			traceutil.RemoveSpanContextFromObject(rs)
		}
