...
func TestValidateOpenTelemetryOptions(t *testing.T) {
	testcases := []struct {
		name        string
		expectError bool
		contents    *OpenTelemetryOptions
	}{
		{
			name:        "nil-valid",
			expectError: false,
		...
		},
		{
			name:        "path-valid",
			expectError: false,
			contents:    &OpenTelemetryOptions{ConfigFile: "/"},
		},
		{
			name:        "path-invalid",
			expectError: true,
			contents:    &OpenTelemetryOptions{ConfigFile: "/path/doesnt/exist"},
		},
	}
