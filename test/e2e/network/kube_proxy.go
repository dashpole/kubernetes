...
var _ = SIGDescribe("Network", func() {
	...
	ginkgo.It("should resolve connrection reset issue #74839 [Slow]", func() {
		...
		_, err := fr.ClientSet.CoreV1().Pods(fr.Namespace.Name).Create(context.Background(), serverPod)
		framework.ExpectNoError(err)
		...
		_, err = fr.ClientSet.CoreV1().Pods(fr.Namespace.Name).Create(context.Background(), pod)
		framework.ExpectNoError(err)
		...
		for i := 0; i < 20; i++ {
			...
			resultPod, err := fr.ClientSet.CoreV1().Pods(fr.Namespace.Name).Get(context.Background(), serverPod.Name, metav1.GetOptions{})
			framework.ExpectNoError(err)
